	...
	<xsl:template match="/">
		...
		<xsl:variable name="ProjectKPIs">
			<table style="width:1250px">
				<tr>
					<xsl:variable name="tollgateKPI" select="//NewDataSet/Projects/*/*/*/@ows_ProjectStatusKPI"/>
					<td>Overall Project:  <xsl:value-of select="//NewDataSet/Projects/*/*/*/@ows_ProjectStatus"/> - <xsl:value-of select="//NewDataSet/KPILevels/*/*/*[@ows_ID = $tollgateKPI]/@ows_Title"/></td>
					<xsl:for-each select="//NewDataSet/ProjectStatusKPIs/*/*/*">
						...
						<td><xsl:value-of select="//NewDataSet/KPICategories/*/*/*[@ows_ID = $thisKPI]/@ows_Title"/> - <xsl:value-of select="//NewDataSet/KPILevels/*/*/*[@ows_ID = $thisLevel]/@ows_Title"/></td>
					</xsl:for-each> 
				</tr>
			</table>	
		</xsl:variable>
		<xsl:variable name="ProjectTeam">
			<table id="ProContacts" class="tablesorter-default">
			 ...
			 <xsl:for-each-group select="//TeamMembers/cw:listitems/rs:data/z:row" group-by="@ows_SharePointUser">
			    <tr>
			      <td align="center"><xsl:value-of select="substring-after(@ows_SharePointUser,';#')"/></td>
			      <td align="center">
