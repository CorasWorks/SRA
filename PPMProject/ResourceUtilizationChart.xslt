...
<xsl:stylesheet 
	...
	exclude-result-prefixes="rs z cw">
	...
	<xsl:key name="resource" match="//NewDataSet/Tasks/cw:listitems/rs:data/z:row" use="@ows_AssignedTo" />
				...
				<anygantt>
					...
					<resource_chart>
						<resources>
							...
							<xsl:if test="count(//NewDataSet/Tasks/cw:listitems/rs:data/z:row[@ows_AssignedTo = ''])">
								<resource>
	...
	<xsl:template name="outputPeriodElement">
		...
		<actions>
			<action type="call" function="clickChartItemView">
				...
				<arg>{%ListId}</arg>
				<arg>{%UniqueId}</arg>
				<arg>[PPM v3.2]/Actions%20Library/ViewTask.cwad</arg>
			</action>
		</actions>
			<style>
				<tooltip>
			...
			<attributes>
				...
				<attribute name="ID">
  				...
  				<attribute name="ListId">
  					<xsl:value-of select="substring-after(substring-before(//NewDataSet/TasksMetadata/cw:List/@ID, '}'), '{')" />
  				</attribute>
