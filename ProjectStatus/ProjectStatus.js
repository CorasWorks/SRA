...
$(document).ready(function () {
	...
	$('#projectStatusSelect').on('change',function(){
		...
		if($("#projectStatusSelect option:selected").text().indexOf('Request Delete') > -1)
			my.projectVM.ProjectDeleteRequest = true;
		else
			my.projectVM.ProjectDeleteRequest = false;			
	});
		...
		success:function(json){
			...
			$.each(json.NewDataSet.KPIs.KPI, function(i,v){
				my.projectVM.kpis.push(
					...
					Cmd: 'Save',
					title:ko.observable(v.leveltitle),
