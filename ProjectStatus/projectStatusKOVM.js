...
projectVM = function () {
    ...
    self.changeKPILevel = function(item, event){
    	...
    	console.log(my.KPILevels.KPILevel[parseInt(i)]['title']);
...
//    	item.icon(my.KPILevels.KPILevel[parseInt(i)]['icon']).level(my.KPILevels.KPILevel[parseInt(i)]['level']).levelID(my.KPILevels.KPILevel[parseInt(i)]['levelID']);
    	item.title(my.KPILevels.KPILevel[parseInt(i)]['title']).icon(my.KPILevels.KPILevel[parseInt(i)]['icon']).level(my.KPILevels.KPILevel[parseInt(i)]['level']).levelID(my.KPILevels.KPILevel[parseInt(i)]['levelID']);
    ...
    self.update = function(item, event){
		...
		if (self.ProjectDeleteRequest)
		{
			var noticeAction = '../_layouts/CorasWorksApps/CorasWorksApplicationService.ashx?RequestType=ExecuteAction&SiteUrl=[SRA Root]&OutputType=json&ListTitle=Projects&ActionUrl=[SRA Root]/Actions Library/deletenotice.cwad&ItemIds='+self.ProjectID;
			$.get(noticeAction, function(data){});
			self.ProjectDeleteRequest = false;
		}
    }
