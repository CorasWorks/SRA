	...
	<xsl:template match="/">
		...
		-->
			...
			<xsl:for-each select="//Projects/*/*/*">
				...
				<xsl:variable name="thisProjectDate" select="if(@ows_TeamLaunchTarget) then substring-before(@ows_TeamLaunchTarget,'T') else substring-before(@ows_Created,'T')"/>
				<xsl:variable name="ProDiv" select="substring-after(tokenize(substring-after($project/Project/@projectURL,'http://'),'/')[contains(.,'div')],'div')" />
				
				<xsl:if test="not(@ows_BenefitsFull) or @ows_BenefitsFull = '' and $ProDiv=$thisDiv">
					...
					<Data>
						<groupingKey><xsl:value-of select="concat($div,$pro,tokenize($thisProjectDate,'-')[1],$cat)"/></groupingKey>
						<Date><xsl:value-of select="$thisProjectDate"/></Date>
						<Year><xsl:value-of select="tokenize($thisProjectDate,'-')[1]"/></Year>
						<Division id="{$div}"><xsl:value-of select="//Divisions/*/*/*[@ows_ID = $div]/@ows_Title"/></Division>
						<HomeDivision id="{$homeDiv}"><xsl:value-of select="//Divisions/*/*/*[@ows_ID = $div]/@ows_Title"/></HomeDivision>
						<Region><xsl:value-of select="//Regions/*/*/*[@ows_ID = $reg]/@ows_Title"/></Region>
						<Location><xsl:value-of select="//Locations/*/*/*[@ows_ID = $loc]/@ows_Title"/></Location>
						...
						<ProjectID><xsl:value-of select="$pro"/></ProjectID>
						<Methodology><xsl:value-of select="//Maps/*/*/*[@ows_ID = $map]/@ows_Title"/></Methodology>
						<Status><xsl:value-of select="//Statuses/*/*/*[@ows_ID = $status]/@ows_Title"/></Status>
					
						<Category><xsl:value-of select="if(//Categories/*/*/*[@ows_ID = $cat]/@ows_PExReportGroup) then upper-case(replace(substring-after(//Categories/*/*/*[@ows_ID = $cat]/@ows_PExReportGroup,';#'),' ','')) else upper-case(replace(//Categories/*/*/*[@ows_ID = $cat]/@ows_Title,' ',''))"/></Category>
						<CategoryID><xsl:value-of select="if(//Categories/*/*/*[@ows_ID = $cat]/@ows_PExReportGroup) then substring-before(//Categories/*/*/*[@ows_ID = $cat]/@ows_PExReportGroup,';#') else //Categories/*/*/*[@ows_ID = $cat]/@ows_ID"/></CategoryID>						
						...
						<CategoryID><xsl:value-of select="$Benefits/Categories/Category[ID = $cat]/PExID"/></CategoryID>
				...
				<xsl:for-each select="tokenize(@ows_BenefitsFull,$MultiDelimiter)[. != '']">
					...
					<xsl:variable name="ProTDiv" select="$this[3]"/>
					<xsl:if test="$ProTDiv=$thisDiv">
					...
					<Data>
						...
						<Division id="{$div}"><xsl:value-of select="$Benefits/Divisions/Division[ID = $div]/Title"/></Division>

						<HomeDivision id="{$homeDiv}"><xsl:value-of select="$Benefits/Divisions/Division[ID = $homeDiv]/Title"/></HomeDivision>
