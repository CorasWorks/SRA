			...
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<xsl:for-each-group select="$FinancialsGroupedbyProject/Data" group-by="Year">
						...
						<xsl:for-each-group select="current-group()" group-by="Division">
							<Data>
								<GridSort>a-<xsl:value-of select="Division/@title"/>-<xsl:value-of select="distinct-values(current-group()[(Status = 'Proposed' or Status = 'Active') and Division/@id = HomeDivision/@id]/ProjectID)"/>-</GridSort>			
								<Division><xsl:value-of select="Division"/></Division>
								<HomeDivision><xsl:value-of select="HomeDivision"/></HomeDivision>
								<Region></Region>
								<Location></Location>
