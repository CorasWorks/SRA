...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:cw="http://schemas.microsoft.com/sharepoint/soap/" xmlns:rs="urn:schemas-microsoft-com:rowset" xmlns:z="#RowsetSchema" exclude-result-prefixes="xs cw rs z">
	...
	<xsl:variable name="Resources">
			<xsl:variable name="PortFolioURL">%PortFolioURL%</xsl:variable>
			<xsl:variable name="CurrentDivID">
				<xsl:for-each select="//Divisions/*/*/*">
					<xsl:variable name="DivisionURLCHECK">
						<xsl:value-of select="substring-after($PortFolioURL, @ows_URL)" />
					</xsl:variable>
					<xsl:if test="(contains($PortFolioURL, @ows_URL) and starts-with($DivisionURLCHECK, '/')) or @ows_URL = $PortFolioURL">				
						<xsl:value-of select="@ows_ID" />
					</xsl:if>
				</xsl:for-each> 
			</xsl:variable>
			<xsl:variable name="DivPart"><xsl:text>/div</xsl:text><xsl:value-of select="$CurrentDivID"/><xsl:text>/</xsl:text></xsl:variable>
<!-- For projects that do not have team members defined -->				
			<xsl:for-each select="//Projects/*/*/*[not(@ows_TeamMemberDivisions) and contains(@ows_URL, $DivPart) and @ows_TeamLeader!='']">
				...
				<Projects>
					<TeamMember><xsl:value-of select="substring-after(@ows_TeamLeader, ';#')"/></TeamMember>
					<TeamMemberRoles>Team Leader</TeamMemberRoles>
					<RolesTest>
...
<!-- For projects that have team members defined -->			
			<xsl:for-each-group select="//Projects/*/*/*[@ows_TeamMemberDivisions!='']" group-by="tokenize(@ows_TeamMemberDivisions,$MultiDelimiter)">
				<xsl:for-each select="current-group()[not(string-length(current-grouping-key()) = 0) and substring-after(current-grouping-key(), '##') = $CurrentDivID]">
					<xsl:variable name="thisKPI" select="@ows_ProjectStatusKPI"/>
					<xsl:variable name="thisKPIIcon" select="if(@ows_ProjectStatus = 'Active')then substring-before(//KPIs/*/*/*[@ows_ID = $thisKPI]/@ows_Image,',') else 0"/>
					<xsl:variable name="thisKPIIconTitle" select="if(@ows_ProjectStatus = 'Active')then //KPIs/*/*/*[@ows_ID = $thisKPI]/@ows_Title else 0"/>
					
				<Projects>
					<TeamMember><xsl:value-of select="substring-before(current-grouping-key(), '##')"/></TeamMember>
					<TeamMemberRoles>
						<xsl:for-each select="tokenize(@ows_TeamMemberRoles,';#')[substring-before(.,'##') = substring-before(current-grouping-key(), '##')]">
