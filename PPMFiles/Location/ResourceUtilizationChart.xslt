...
<xsl:stylesheet version="2.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cw="http://schemas.microsoft.com/sharepoint/soap/" xmlns:rs="urn:schemas-microsoft-com:rowset" xmlns:z="#RowsetSchema" exclude-result-prefixes="xs cw rs z">
	...
	<xsl:variable name="TaskSet">
		<xsl:copy-of select="//NewDataSet/*/NewDataSet/Projects" />
		<!--
				<xsl:for-each select="//NewDataSet/*/NewDataSet/Tasks">
					<xsl:copy-of select="." />			
				</xsl:for-each>
	...
	<xsl:variable name="assignedToNames">
		<NewDataSet>
			<xsl:for-each select="//NewDataSet/*/NewDataSet/Projects/cw:listitems/rs:data/z:row[count(. | key('resource', @ows_TeamMembers)[1]) = 1 and @ows_TeamMembers != '']">
				<xsl:variable name="multiSelectColumn">
					<xsl:call-template name="assignedToHandler">
						...
						<xsl:with-param name="thisAssignedTo" select="@ows_TeamMembers" />
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="count(//NewDataSet/*/NewDataSet/Projects/cw:listitems/rs:data/z:row[@ows_TeamLaunchTarget]) = 0">
				<anygantt>
					<settings>
						<title align="Center" angle="0" position="Top">
							<text>No Projects with Milestone Dates</text>
			...
			<xsl:otherwise>
				<anygantt>
				  <styles>
					...
					<datagrid>
					    <columns>
					      <column width="300" cell_align="LeftLevelPadding">
					        <header>
					          <text>Name / Project</text>
					        </header>
					        <format>{%Name}</format>
					      </column>
					      <!--
					      <column width="75" cell_align="LeftLevelPadding">
					        <header>
					          <text>First Milestone</text>
					        </header>
					        <format>{%Start}</format>
					      </column>
					      <column width="75" cell_align="LeftLevelPadding">
					        <header>
					          <text>Last Milestone</text>
					        </header>
					        <format>{%End}</format>
					      </column>
					      --> 
					...
					<resource_chart>
						<resources>
							...
							<xsl:for-each select="$assignedTo/AssignedTo">
								...
								<xsl:for-each select="$TaskSet/Projects/cw:listitems/rs:data/z:row[contains(@ows_TeamMembers,$UserFull)]">
										<xsl:variable name="siteURL" select="@ows_URL" />
										<xsl:variable name="SiteTitle" select="@ows_Title" />
										<resource>
											<xsl:attribute name="id">
						...
						<periods>
						<!-- for signal line -->
							<xsl:for-each select="$TaskSet/Projects">
								<xsl:for-each select="cw:listitems/rs:data/z:row">
									<xsl:variable name="siteURL" select="@ows_URL" />
										<xsl:call-template name="outputPeriodElement">
											...
											<xsl:with-param name="projectTitle" select="@ows_Title" />
										</xsl:call-template>
								</xsl:for-each>
							</xsl:for-each>
						<!-- for groups -->
	...
	<xsl:template name="outputPeriodElement">
		...
		<xsl:if test="@ows_TeamLaunchTarget">
		<period>
			...
			<xsl:if test="@ows_TeamLaunchTarget != ''">
				<xsl:attribute name="start">
  				<xsl:value-of select="translate(substring(@ows_TeamLaunchTarget,1,10),'.','/')"/>
  				<xsl:text>&#160;</xsl:text> 
  				<xsl:value-of select="translate(substring(@ows_TeamLaunchTarget,12,5),'.',':')"/>
  			</xsl:attribute>
				<xsl:if test="@ows_FinalMilestoneTarget = ''">
			...
			<xsl:if test="@ows_FinalMilestoneTarget != ''">
				<xsl:attribute name="end">
				<xsl:value-of select="translate(substring(@ows_FinalMilestoneTarget,1,10),'.','/')"/>
  				<xsl:text>&#160;</xsl:text> 
  				<xsl:value-of select="translate(substring(@ows_FinalMilestoneTarget,12,5),'.',':')"/>
			</xsl:attribute>
				<xsl:if test="@ows_TeamLaunchTarget = ''">
					<xsl:attribute name="start">
	  				<xsl:value-of select="translate(substring(@ows_FinalMilestoneTarget,1,10),'.','/')"/>
...
First Milestone Target: <xsl:value-of select="substring-before(@ows_TeamLaunchTarget, ' ')" />
Last Milestone Target: <xsl:value-of select="substring-before(@ows_FinalMilestoneTarget, ' ')" />
<!--
First Milestone Actual: <xsl:value-of select="substring-before(@ows_TeamLaunchActual, ' ')" />
Last Milestone Actual: <xsl:value-of select="substring-before(@ows_FinalMilestoneActual, ' ')" />
-->
  				</text>
				</tooltip>
				<bar_style>
