...
(function($){
	...
	$.get("../Board/settings.xml",function(xml){
	...
	var taskItemModel = function(){
		...
		this.actualStart = ko.observable();
		this.actualFinish = ko.observable();
		this.status = ko.observable();
		this.work = ko.observable();
		this.actualWork = ko.observable();
		...
		this.assignedToIcon = ko.computed(function(){
			...
			if (this.assignedTo() != null && this.assignedTo().length > 0){
				...
				if (isMe) { icon = "/CWPPM32/Images/hat_green.png"; this.assignedToToolTip("My Task"); }
				else { this.assignedToToolTip("Assigned"); icon = "/CWPPM32/Images/hat_gray.png"; }
			}
			else { 
		...
		this.itemBatchData = function(){
			...
			if (this.startDate()) { $.extend(items[0], { StartDate: cw.utils.formatDateForSave(this.startDate()) }); }
			if (this.dueDate())	{ $.extend(items[0], { DueDate: cw.utils.formatDateForSave(this.dueDate()) }); }
			if (this.actualStart()) {$.extend(items[0], { ActualStart: cw.utils.formatDateForSave(this.actualStart()) }); }
			if (this.actualFinish()) {$.extend(items[0], { ActualFinish: cw.utils.formatDateForSave(this.actualFinish()) }); }
		...
		this.formatWorkValue = function(value){
			try{
	...
	var BoardViewModel = function(config){
		...
		this.showFilterByUser = ko.observable(false);
