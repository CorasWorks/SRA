...
<xsl:stylesheet version="2.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cw="http://schemas.microsoft.com/sharepoint/soap/" xmlns:rs="urn:schemas-microsoft-com:rowset" xmlns:z="#RowsetSchema" exclude-result-prefixes="xs cw rs z">
	...
	<div id="PCDisplayRoot">
		<div id="SiteManagerLoader" style="text-align: center; display:none">
		...
		<div id="SiteManagerShell">
			<div id="SiteManagerWrap">
				<!-- pager -->
				<div id="PCDocumentsPager" class="pagerSiteManager">
				  <form>
				...
				<table id="SiteManager" class="tablesorter-default">
				  <thead>
				    <tr>
				  ...
				  <tbody>
				  <xsl:for-each select="//Divisions/cw:listitems/rs:data/z:row">
				  <xsl:variable name="CurrentURL" select="@ows_URL" />
 				  <xsl:variable name="TotalSites" select="count(//Regions/cw:listitems/rs:data/z:row[contains(@ows_URL,$CurrentURL)])" />
				  <!--
 				  <xsl:variable name="TotalSites" select="count(//Regions/cw:listitems/rs:data/z:row[contains(@ows_URL,$CurrentURL)]) + count(//Locations/cw:listitems/rs:data/z:row[contains(@ows_URL,$CurrentURL)]) + count(//Projects/cw:listitems/rs:data/z:row[contains(@ows_URL,$CurrentURL)])" />
				  -->	
					<xsl:variable name="DeleteSite">
						<button class="DivisionDelete" data-id="{@ows_ID}" data-url="{@ows_URL}"  data-title="{@ows_Title}" data-usergroup="{@ows_UserGroupID}">
					...
					<xsl:variable name="EditSite">
						<xsl:if test="@ows_UserGroupID">
							<button class="DivisionEdit" data-title="{@ows_Title}" data-url="{@ows_URL}">Edit</button>
						</xsl:if>
					</xsl:variable>
 					<xsl:variable name="Activate">
						<xsl:if test="not(@ows_UserGroupID)">				
							<button class="DivisionActivate" data-id="{@ows_ID}" data-parentid="{@ows_ParentID}" data-title="{@ows_Title}" data-url="{@ows_URL}">
						...
						<tr>
							...
							<!--
							<td><xsl:text>Division</xsl:text></td>
