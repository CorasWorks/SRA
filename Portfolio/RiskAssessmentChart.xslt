...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template match="/">
		<xsl:choose>
			<xsl:when test="sum(//NewDataSet/*/NewDataSet/Risks/cw:listitems/rs:data/@ItemCount) = 0">
				<anychart>
					<charts>
						<chart plot_type="Pie">
							<chart_settings>
							...
							<data>
								<actions>
									<action type="call" function="clickChartItemView">
										...
										<arg>{%ListId}</arg>
										<arg>{%UniqueId}</arg>
										<arg>[PPM v3.2]/Actions%20Library/ViewRiskPortfolio.cwad</arg>
									</action>
								</actions>
								<xsl:for-each select="//NewDataSet/*/NewDataSet/Risks">
									<series type="Marker">
										<xsl:attribute name="name">
											<xsl:variable name="thisSiteURL" select="@SiteUrl" />
											
											<xsl:value-of select="//NewDataSet/*/NewDataSet/SiteDirectory/cw:listitems/rs:data/z:row[@ows_URL = $thisSiteURL]/@ows_Title" />
										</xsl:attribute>
										<xsl:for-each select="cw:listitems/rs:data/z:row">
											<point>
												<xsl:attribute name="x">
									  				<xsl:value-of select="substring-before(@ows_Probability, '.')"/>
									  			</xsl:attribute>
												<xsl:attribute name="y">
									  				<xsl:value-of select="substring-before(@ows_Impact, '.')"/>
									  			</xsl:attribute>
												<xsl:attribute name="name">
